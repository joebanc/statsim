// ImportAutoSelDlg.cpp : implementation file
//
...
extern void ImportData(CString strPath, CStatSimConn* pToDB, long ID);
extern void ImportData(CString strPath, long ID);

extern CMFCRibbonStatusBarPane *g_wndStatusBarPane;
extern CProgressCtrl  m_ProgStatBar;

DWORD hlimit = MUN, hLstLimit = BRGY;
...
IMPLEMENT_DYNAMIC(CImportAutoSelDlg, CDialog)

CImportAutoSelDlg::CImportAutoSelDlg(long curr_qnrID, CWnd* pParent /*=NULL*/)
	: CDialog(CImportAutoSelDlg::IDD, pParent)
{
...
CImportAutoSelDlg::~CImportAutoSelDlg()
{	
...
void CImportAutoSelDlg::DoDataExchange(CDataExchange* pDX)
{
...
BEGIN_MESSAGE_MAP(CImportAutoSelDlg, CDialog)
	ON_BN_CLICKED(IDC_SPECIFYPATH, &CImportAutoSelDlg::OnBnClickedSpecifypath)
//	ON_NOTIFY(TVN_SELCHANGED, IDC_TREELEVEL, &CImportAutoSelDlg::OnTvnSelchangedTreelevel)
	ON_BN_CLICKED(IDC_SELECT, &CImportAutoSelDlg::OnBnClickedSelect)
	ON_BN_CLICKED(IDC_DESELECT, &CImportAutoSelDlg::OnBnClickedDeselect)
	ON_BN_CLICKED(IDC_SELECTALL, &CImportAutoSelDlg::OnBnClickedSelectall)
	ON_BN_CLICKED(IDC_DESELECTALL, &CImportAutoSelDlg::OnBnClickedDeselectall)
	ON_BN_CLICKED(IDOK, &CImportAutoSelDlg::OnBnClickedOk)
	ON_NOTIFY(NM_DBLCLK, IDC_LISTUNSEL, &CImportAutoSelDlg::OnNMDblclkListunsel)
	ON_NOTIFY(NM_DBLCLK, IDC_LISTSEL, &CImportAutoSelDlg::OnNMDblclkListsel)
	ON_BN_CLICKED(IDC_VIEW, &CImportAutoSelDlg::OnBnClickedView)
	ON_BN_CLICKED(IDC_CHANGE, &CImportAutoSelDlg::OnBnClickedChange)
	ON_NOTIFY(NM_DBLCLK, IDC_TREELEVEL, &CImportAutoSelDlg::OnNMDblclkTreelevel)
END_MESSAGE_MAP()


// CImportAutoSelDlg message handlers

void CImportAutoSelDlg::OnBnClickedSpecifypath()
{
	OnCancel();

	CImportSelPathDlg* pImportSelPathDlg;
	pImportSelPathDlg = new CImportSelPathDlg(m_qnrID);

	pImportSelPathDlg->DoModal();
}

void CImportAutoSelDlg::FillUnSel() 
{
	...
	int nCols = 2;
	LPCSTR* sCols = (LPCSTR*) malloc(nCols * sizeof(LPCSTR));
	...
	if (hselItem!=NULL) {
		...
		if(hclimit == hlimit) {
			...
			if (plistRS->GetRecordCount() > 0) {
				plistRS->MoveFirst();
				lstTable = plistRS->SQLFldValue(_MBCS("element"));
				lstlabelFld = plistRS->SQLFldValue(_MBCS("name"));
				lstIDFld = plistRS->SQLFldValue(_MBCS("IDField"));
			}
			...
			if (plistRS->GetRecordCount() > 0) {
				plistRS->MoveFirst();
				lstDigit = plistRS->SQLFldValue(_MBCS("digit"));
			}
			
			if (_ttoi(lstDigit) > 0) {
				lstcritFld.Format(_T("Left(%s, %s)"), lstIDFld, lstDigit);
				lstcritValue.Format(_T("%d"), _ttol(strID.Left(_ttoi((lstDigit)))));
			}
...
int CImportAutoSelDlg::AutoImport(CStatSimList* pSrcList)
{
	try
	{
		...
		for (int i=0;i < k;i++)
		{
			...
			statusText.Format(_T("Importing %s: %s"), currLevel, currPath2);
			LPCSTR sBatchDB;
...
check:		if ( FileFinder.FindFile( currPath1) )
			{
				ffound = 1;
				currPath = currPath1;
			}
			else if ( FileFinder.FindFile(currPath2) )
			{
				ffound = 1;
				currPath = currPath2;
			}
			else if ( FileFinder.FindFile( currPath3) )
			{
				ffound = 1;
				currPath = currPath3;
			}
			...
			if ( !ffound ) {
				...
				if (!IsDirectProc) {
					...
					sBatchDB = NULL;		
					arrayBatchDB[i] = (LPCSTR) sBatchDB;
				}
...
void CImportAutoSelDlg::OnBnClickedSelect()
{
...
void CImportAutoSelDlg::OnBnClickedDeselect()
{
...
void CImportAutoSelDlg::OnBnClickedSelectall()
{
...
void CImportAutoSelDlg::OnBnClickedDeselectall()
{
...
void CImportAutoSelDlg::OnBnClickedOk()
{
	CString msgText;
	int msgResult;
	CDialog::OnOK();
...
void CImportAutoSelDlg::OnNMDblclkListunsel(NMHDR *pNMHDR, LRESULT *pResult)
{
...
void CImportAutoSelDlg::OnNMDblclkListsel(NMHDR *pNMHDR, LRESULT *pResult)
{
	OnBnClickedDeselect();
	*pResult = 0;
}

BOOL CImportAutoSelDlg::OnInitDialog()
{
	CDialog::OnInitDialog();

	//temporary since no handler for autoselection in BPQ
	if (m_qnrID!=hpq_id) {
		AfxMessageBox(_T("This feature is not yet available for BPQ's"));
		OnBnClickedSpecifypath();
	}
...
void CImportAutoSelDlg::OnBnClickedView()
{

	ULONG nItems = 0;
	int k = hlimit-(1001), // prov=1003; minus 1001; index is 2 //hay, this is bad (1000)
		d = atol(m_pElt->Attr(digit));
	vector<LPCSTR> *sItems = new vector<LPCSTR>[m_nListJ]; //Name, ID, path
	...
	for (int i=0; i<hSizeArray[k]; i++) { 
		...
		if (nStatus == CStatSimTree::CHECKED) {
			sSQL.Format( _T("SELECT * FROM `%s` WHERE LEFT(`%s`, %d)='%s';"), 
				(CString) m_pElt->ChiAttr(element), (CString) m_pElt->ChiAttr(IDField), d, sCrit );
...
//void CImportAutoSelDlg::OnTvnSelchangedTreelevel(NMHDR *pNMHDR, LRESULT *pResult)
//{
...
void CImportAutoSelDlg::OnBnClickedChange()
{
	m_TreeLevel.EnableWindow();
	m_ListSelect.CleanUp(m_sListCols, m_nListJ, FALSE, EMPTY);
	m_ListUnselect.CleanUp(m_sListCols, m_nListJ, FALSE, EMPTY);
...
check:		if ( !FileFinder.FindFile((LPCSTR) currPath) ) {
				
				
				if (!IsDirectProc) {
					arrayProcExist.InsertAt(i, FALSE);	//add array of boolean
					sBatchDB = NULL;		
					arrayBatchDB.InsertAt(i, (LPCSTR) sBatchDB);
				}
...
*/
void CImportAutoSelDlg::OnNMDblclkTreelevel(NMHDR *pNMHDR, LRESULT *pResult)
{
